...
# Determine the PostgreSQL admin user.
if psql -h "$DB_HOST" -p "$DB_PORT" -U postgres -d postgres -c '\q' >/dev/null 2>&1; then
  PG_ADMIN_USER="postgres"
elif psql -h "$DB_HOST" -p "$DB_PORT" -U "$USER" -d postgres -c '\q' >/dev/null 2>&1; then
  PG_ADMIN_USER="$USER"
else
  PG_ADMIN_USER="postgres" # fallback
fi
...
psql -h "$DB_HOST" -p "$DB_PORT" -U "$PG_ADMIN_USER" -d postgres -tc \
  "SELECT 1 FROM pg_roles WHERE rolname = '$DB_USER'" \
  | grep -q 1 \
  || psql -h "$DB_HOST" -p "$DB_PORT" -U "$PG_ADMIN_USER" -d postgres -c \
...
echo "   2. Run 'npx prisma migrate dev' to sync the schema"
